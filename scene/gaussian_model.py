...
class GaussianModel:
    ...
    # def densify_and_clone(self, grads, grad_threshold, scene_extent, density_threshold=20, displacement_scale=20, model_path=None, iteration=None, stage=None):
    #     """
    #     Densifies the point cloud by cloning points that meet specific gradient and density conditions.

    #     Args:
    #         grads (Tensor): Gradient accumulation tensor for points.
    #         grad_threshold (float): Threshold for selecting points based on gradients.
    #         scene_extent (float): Extent of the scene for density filtering.
    #         density_threshold (float, optional): Minimum density of points to be cloned. Default is 20.
    #         displacement_scale (float, optional): Scaling factor for the displacement. Default is 20.
    #         model_path (str, optional): Path for saving intermediate results. Default is None.
    #         iteration (int, optional): Current iteration. Default is None.
    #         stage (str, optional): Current training stage. Default is None.

    #     Returns:
    #         int: Number of new points added.
    #     """
    #     # Mask points based on gradient threshold
    #     grads_norm = torch.norm(grads, dim=-1)  # Compute norms
    #     grads_accum_mask = grads_norm >= grad_threshold  # Points with high gradients

    #     # Additional filtering based on scaling and scene density
    #     scaling_max = torch.max(self.get_scaling, dim=1).values
    #     density_filter_mask = scaling_max <= self.percent_dense * scene_extent
    #     selected_pts_mask = torch.logical_and(grads_accum_mask, density_filter_mask)

    #     # If no points are selected, return early
    #     if not selected_pts_mask.any():
    #         print("[INFO] No points selected for densification at iteration:", iteration)
    #         return 0

    #     # Extract selected points and their properties
    #     new_xyz = self._xyz[selected_pts_mask]
    #     new_features_dc = self._features_dc[selected_pts_mask]
    #     new_features_rest = self._features_rest[selected_pts_mask]
    #     new_opacities = self._opacity[selected_pts_mask]
    ...
    #     # Postfix the new points to the current dataset
    #     self.densification_postfix(
    #         new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_deformation_table
    #     )

    #     # Log the number of new points added
    #     num_new_points = new_xyz.shape[0]
    #     print(f"[INFO] Added {num_new_points} new points during densification (iteration {iteration}).")
